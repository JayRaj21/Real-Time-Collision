...
BG       = "#1a1a1a"
PANEL_BG = "#242424"
ACCENT   = "#00bcd4"   # teal to distinguish from YOLO green
MUTED    = "#777777"
WHITE    = "#f0f0f0"
...
def _ap_from_confs(confs: list) -> float:
    ...
    return sum((k + 1) * c for k, c in enumerate(ranked)) / sum(range(1, len(ranked) + 1))
...
class DetectionApp:
    def __init__(self, root: tk.Tk, use_csi: bool = False,
        ...
        self.root.title("RT-DETR Object Detection — GPU")
    ...
    def _build_ui(self) -> None:
        ...
        self._row(right, "Type",   "RT-DETR (NMS-free)", static=True)
    ...
    def _inference_loop(self) -> None:
        ...
        while self.running:
            ...
            results = self.model.predict(
                frame, device=DEVICE, conf=CONF_THRESHOLD, imgsz=480, half=True, verbose=False
            )
            r = results[0]
            annotated = r.plot()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="RT-DETR GPU object detection")
    ...
    if not torch.cuda.is_available():
        ...
        print("Please run detect_rtdetr_cpu.py instead (expect ~1-3 FPS).")
        sys.exit(1)
