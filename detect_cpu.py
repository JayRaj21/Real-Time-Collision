...
class _RollingMetrics:
    ...
    def __init__(self, window: int = ROLLING_WINDOW) -> None:
        self._prec:     deque = deque(maxlen=window)  # per-frame mean confidence
        self._has_det:  deque = deque(maxlen=window)  # 1.0 if frame had detections
        self._class_ap: deque = deque(maxlen=window)  # per-frame mean class AP

    def update(self, confs: list, class_ids: list) -> dict:
        has = len(confs) > 0
        frame_prec = sum(confs) / len(confs) if has else 0.0

        # Per-class AP for this frame
        if has:
...
class DetectionApp:
    def __init__(self, root: tk.Tk, use_csi: bool = False,
        ...
        self.root.title("YOLO26 Object Detection — CPU")
    ...
    def _build_ui(self) -> None:
        ...
        self._row(right, "Device",  DEVICE.upper(), static=True)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="YOLO26 CPU object detection")
